...
function switchTab(tabId) {
    ...
    else if (tabId === 'mensajes-tab') { loadUsersForChat(); loadPredefinedAssets(); loadQuickMessages(); }
    ...
    chatDiv.innerHTML = messages.map(m => {
        ...
        if (m.attachment_url) {
            ...
            if (isImage) {
                ...
                preview = `<div class="mt-2"><a href="${m.attachment_url}" target="_blank" class="btn btn-sm btn-outline-info">📎 Ver archivo</a></div>`;
            }
        }
...
function selectQuickMessage(msgId) {
    ...
    let text;
    if (lang === 'en') text = msg.text_en;
    else if (lang === 'pt') text = msg.text_pt;
    else text = msg.text_es;
